...
void KIGPDialog::setupLookPage(const QString& path) {
    ...
    m_fontName->setCurrentText( look.readEntry("FontName", KGlobalSettings::generalFont().family() ) );
...
void KIGPDialog::setupThumbnailPage(const QString& path) {
    ...
    m_imageFormat->setCurrentText( group.readEntry("ImageFormat", "JPEG") );
    ...
    m_colorDepth->setCurrentText(group.readEntry("ColorDepth", "8"));
